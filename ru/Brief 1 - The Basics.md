...
Циклы `for...in` и `for...of` используют синтаксис ES6 для перебор массива по ключу или значению.
...
>1. Использование типа `any` считается плохой практикой и его следует применять с осторожностью. Лучшая практика - это использовать конструкцию языка `interface` для определения пользовательских типов.  
...
пример функции, которая принимает объект
...
Объект может быть явно приведен к другому типу объекта с использованием синтаксиса `<>` или `as` в правой части выражения.
...
setGlobalString();
console.log(globalString);  // Ошибка компиляции!
...
>1. Утверждение присваивания не является хорошей практикой и его следует избегать. Если подобная проблема возникает, следует пересмотреть логику и архитектуру приложения.
