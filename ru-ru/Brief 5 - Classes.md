...
> Поскольку классы JavaScript являются функциями которые можно вызывать с оператором `new`, есть зарезервированные слова которые нельзя применять в названии статических методов. Такие как `name`, `length` и `call`, ....
